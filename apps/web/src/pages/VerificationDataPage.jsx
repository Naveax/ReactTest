...
export default function VerificationDataPage() {
  ...
  return (
    <section className="space-y-6">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-board">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-board">
        <h3 className="font-display text-xl font-semibold text-slate-900">
          Created Certificates
        </h3>
        {data.createdCertificates.length === 0 ? (
          <p className="mt-3 text-sm text-slate-600">No created certificate records yet.</p>
        ) : (
          <div className="mt-4 space-y-3">
            {data.createdCertificates.map((item) => (
              <article
                className="rounded-2xl border border-cyan-200 bg-cyan-50/60 p-4"
                key={item.id}
              >
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-board">
        ...
        {data.verificationChecks.length === 0 ? (
          <p className="mt-3 text-sm text-slate-600">No verification checks yet.</p>
        ) : (
          <div className="mt-4 space-y-3">
